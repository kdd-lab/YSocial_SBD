...
def create_app(db_type="sqlite", desktop_mode=False):
    ...
    with app.app_context():
        ...
        try:
            # Run migration to add per-user limits columns to admin_users if needed
            if db_type == "sqlite":
                from y_web.migrations.add_user_limits_columns import (
                    migrate_sqlite as migrate_user_limits_sqlite,
                )

                dashboard_db_path = app.config.get("DASHBOARD_DB_PATH")
                if dashboard_db_path:
                    migrate_user_limits_sqlite(dashboard_db_path)
            elif db_type == "postgresql":
                from y_web.migrations.add_user_limits_columns import (
                    migrate_postgresql as migrate_user_limits_postgresql,
                )
                ...
                pg_password = os.getenv("PG_PASSWORD", "")
                if pg_password:
                    migrate_user_limits_postgresql(
                        pg_host, pg_port, pg_database, pg_user, pg_password
                    )
        except Exception as e:
            print(f"Failed to run admin user limits columns migration: {e}")

        try:
            # Run migration to add per-user client configuration limits if needed
            if db_type == "sqlite":
                from y_web.migrations.add_client_config_limits_columns import (
                    migrate_sqlite as migrate_client_limits_sqlite,
                )

                dashboard_db_path = app.config.get("DASHBOARD_DB_PATH")
                if dashboard_db_path:
                    migrate_client_limits_sqlite(dashboard_db_path)
            elif db_type == "postgresql":
                from y_web.migrations.add_client_config_limits_columns import (
                    migrate_postgresql as migrate_client_limits_postgresql,
                )
                ...
                pg_password = os.getenv("PG_PASSWORD", "")
                if pg_password:
                    migrate_client_limits_postgresql(
                        pg_host, pg_port, pg_database, pg_user, pg_password
                    )
        except Exception as e:
            print(f"Failed to run client configuration limits migration: {e}")
