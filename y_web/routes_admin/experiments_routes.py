...
def experiment_details(uid):
    ...
    if experiment and experiment.simulator_type == "HPC":
        ...
        if bind_key not in current_app.config["SQLALCHEMY_BINDS"]:
            current_app.logger.error(
                f"Failed to register experiment database for exp_id={uid}"
            )
            # Continue without HPC user management - experiment can still be viewed
        else:
                ...
                if owner_admin:
                    ...
                    if not existing_user:
                        ...
                        try:
                            ...
                            db.session.rollback()
                            current_app.logger.warning(
                                f"Failed to add owner to HPC experiment database: {e}"
                            )
                ...
                if current_user.username != experiment.owner:
                    ...
                    if not existing_current_user:
                        ...
                        try:
                            ...
                            db.session.rollback()
                            current_app.logger.warning(
                                f"Failed to add current user to HPC experiment database: {e}"
                            )
            finally:
                # Restore original bind unconditionally
